...
const ModerateClubs = () => {
  ...
  async function getPosts() {
      const config = {
        method: 'get',
        url: 'http://localhost:8000/moderateclubs.php'
      }
    ...
    function delPostsForm(delClub) {
        ...
        axios({
          ...
          data: { delClub : delClub }
        })
          .then(result => {
            if (result.data === "success") {
              alert("clubs" + delClub + " is  deleted");
              
            }
            else {
              alert("clubs " + delClub + " already deleted.");
...
<div className="mb-main-div">
  <div className="main-title">Moderate Posts</div>
      ...
      <div className="mb-content-div">
        <div className="mb-single-option">
          <div className="delete-advertisement">
            <div className="mb-sub-content">
              <div>
                <div className="sub-title">ModerateClubs</div>
                ...
                <table className="styled-table" id="adv-table">
                  ...
                  {PostsList.length > 0 ? PostsList.map((i,index)=>
                      <tr key={index}>
                        <td >{i.clubName}</td>
                        <td>{i.clubDesc}</td>
                        <td>{i.memberscount}</td>
                      
                        
                        <td> <button type="buy" className="mma-buy-btn" onClick={() => delPostsForm(i.clubName)} >Delete club</button>
                          </td>
